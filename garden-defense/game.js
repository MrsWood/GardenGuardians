const canvas = document.getElementById("game");
    ...
    function syncTowerSelectionUI() {
      ...
      sprayTowerBtn.setAttribute("aria-pressed", selectedTowerType === "spray" ? "true" : "false");
      glueTowerBtn.setAttribute("aria-pressed", selectedTowerType === "glue" ? "true" : "false");
      hoseTowerBtn.setAttribute("aria-pressed", selectedTowerType === "hose" ? "true" : "false");
    ...
    function renderSaltTowerIcon() {
      if (!saltTowerBtn) return;
      const saltPreviewHost = saltTowerBtn.querySelector(".saltPreview");
      if (!saltPreviewHost) return;

      let iconCanvas = saltPreviewHost.querySelector("canvas");
      if (!iconCanvas) {
        iconCanvas = document.createElement("canvas");
        iconCanvas.width = 120;
        iconCanvas.height = 82;
        iconCanvas.className = "saltPreviewCanvas";
        saltPreviewHost.textContent = "";
        saltPreviewHost.appendChild(iconCanvas);
      }
      ...
      // Match the in-game idle glow beneath the cannon.
      iconCtx.fillStyle = "rgba(241, 244, 250, 0.2)";
      iconCtx.beginPath();
      iconCtx.arc(52, 48, 16, 0, Math.PI * 2);
      iconCtx.fill();

      iconCtx.save();
      iconCtx.translate(52, 48);
      iconCtx.scale(1.5, 1.5);
    ...
    function drawTowers() {
      for (const t of towers) {
        ...
        if (t.type === "glue") {
          ...
          // Noxious gas-cloud tower body to match the spray selector icon.
          ...
          ctx.strokeStyle = "rgba(112, 24, 24, 0.62)";
          ctx.lineWidth = 0.9;
          ctx.beginPath();
          ctx.arc(-1.2, 0.4, 8.5, -2.65, 1.05);
          ctx.stroke();

          // Small pale core like selector detail.
          ctx.fillStyle = "rgba(255, 222, 222, 0.86)";
          ctx.beginPath();
          ctx.arc(-0.2, -4.6, 1.6, 0, Math.PI * 2);
          ctx.fill();

          // Short emitter tip to preserve aim readability.
          ...
          if (t.sprayFlash > 0) {
            ...
            // Fan arcs improve spray direction readability.
    ...
    function drawEnemies() {
      ...
      for (const e of enemies) {
        ...
        if (!compactHud) {
          ...
          if ((e.slowFlash || 0) > 0) {
            ctx.fillStyle = "rgba(147, 227, 255, 0.95)";
            ctx.font = "700 7px Segoe UI";
            ctx.textAlign = "right";
            ctx.fillText("SLOW", panelX + panelW - 3, panelY + 4.6);
          }
        }
      }
    }
    ...
    sprayTowerBtn.addEventListener("keydown", (e) => towerSelectKeys(e, "spray"));
    glueTowerBtn.addEventListener("keydown", (e) => towerSelectKeys(e, "glue"));
    hoseTowerBtn.addEventListener("keydown", (e) => towerSelectKeys(e, "hose"));
